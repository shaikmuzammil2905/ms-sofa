...
<script>
    ...
    function scrollToCategorySection(targetId) {
        ...
        var header = document.querySelector('header.header-sticky') || document.querySelector('header');
        var jumpBar = document.querySelector('.sticky-jump-bar') || document.querySelector('.filter-anchor-bar');
        var headerHeight = header ? header.offsetHeight : 70;
        var jumpBarHeight = jumpBar ? jumpBar.offsetHeight : 60;
        var totalOffset = headerHeight + jumpBarHeight + 20;

        var elemTop = targetEl.getBoundingClientRect().top + window.pageYOffset;
        window.scrollTo({
            top: elemTop - totalOffset,
            behavior: 'smooth'
        });

        document.querySelectorAll('.category-jump-btn').forEach(function(b) {
            var href = b.getAttribute('href') || '';
            if (href.endsWith('#' + targetId) || href === '#' + targetId) {
    ...
    document.addEventListener('click', function(e) {
        var btn = e.target.closest('.category-jump-btn');
        if (!btn) return;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (window.location.hash) {
            ...
            setTimeout(function() { scrollToCategorySection(hashTarget); }, 300);
        }
    });
    window.addEventListener('load', function() {
        ...
        if (window.location.hash) {
            var hashTarget = window.location.hash.replace('#', '');
            setTimeout(function() { scrollToCategorySection(hashTarget); }, 150);
        }
    });
</script>
